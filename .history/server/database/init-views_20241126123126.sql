-- Drop existing views
IF EXISTS (SELECT * FROM sys.views WHERE name = 'vw_breakout_analysis')
    DROP VIEW vw_breakout_analysis
GO

IF EXISTS (SELECT * FROM sys.views WHERE name = 'vw_technical_analysis')
    DROP VIEW vw_technical_analysis
GO

IF EXISTS (SELECT * FROM sys.views WHERE name = 'vw_volume_analysis')
    DROP VIEW vw_volume_analysis
GO

IF EXISTS (SELECT * FROM sys.views WHERE name = 'vw_latest_prices')
    DROP VIEW vw_latest_prices
GO
...
CREATE VIEW vw_volume_analysis AS
WITH LatestData AS (
  SELECT 
    ticker,
    date,
...
CREATE VIEW vw_technical_analysis AS
WITH LatestData AS (
  SELECT 
    ticker,
    date,
...
CREATE VIEW vw_breakout_analysis AS
WITH PriceData AS (
  SELECT 
    ticker,
    date,

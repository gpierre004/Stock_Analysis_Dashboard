...
SELECT 
  MAX("CompanyTicker") as "CompanyTicker",  -- Use aggregation for non-grouped columns
  FIRST_VALUE("adjustedClose") OVER (ORDER BY date DESC) as current_price,  -- Get most recent price
  ROUND(
    (FIRST_VALUE("adjustedClose") OVER (ORDER BY date DESC) - FIRST_VALUE(price_20d_ago) OVER (ORDER BY date DESC)) 
    / NULLIF(FIRST_VALUE(price_20d_ago) OVER (ORDER BY date DESC), 0) 
    * 100, 
    2
  ) as price_change_20d,

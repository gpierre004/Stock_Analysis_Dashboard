...
function StockCard({ ticker }: { ticker: string }) {
  ...
  const { volume, avg_volume: avgVolume, vwap } = volumeData[0];
  const { current_price: price, price_change_20d: change, sma20 } = technicalData[0];
  ...
  return (
    <>
      ...
      <TechnicalSignals
        ticker={ticker}
        support={price * 0.95} // Simplified support level
        resistance={price * 1.05} // Simplified resistance level
        rsi={50} // This should be calculated properly
